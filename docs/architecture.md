...
This document outlines the early architecture that satisfies the PRD and Phase 0 deliverables. It is intentionally implementation-neutral while mapping the data loop between the XR app, PixelBrain/OS, and Continuon Cloud.
...
- **PixelBrain/OS bridge (dock/phone):** Receives gRPC/WebRTC streams from XR client for robot state, commands, and telemetry. Exposes the stable Robot API.
- **Continuon Cloud:** Stores RLDS episodes, runs training/validation jobs, and manages OTA updates to PixelBrain/OS and XR client configs.
...
- **Input devices:** Headset pose, hand tracking, Continuon Glove BLE stream; input fusion and synchronization.
- **Data logging:** RLDS episode builder enforcing schema; local persistence; upload/forwarder to Cloud.
- **Connectivity:** gRPC/WebRTC client to PixelBrain/OS; HTTPS/WebSocket client to Cloud; BLE manager for glove.
...
1. XR captures headset/hand poses and glove frames at ~100 Hz.
2. Teleop adapter maps inputs to normalized robot actions.
3. Bridge sends commands to PixelBrain/OS over gRPC/WebRTC; receives robot state feedback.
4. RLDS logger syncs observation (poses, glove, video, robot state) with action and timestamps; stores locally.
...
- Teleop to mock PixelBrain/OS instance (local service).
