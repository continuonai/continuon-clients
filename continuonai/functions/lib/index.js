...
exports.training = exports.remote = exports.rlds = exports.trainingUpdate = exports.trainingCreate = exports.remoteSession = exports.remoteVerify = exports.rldsList = exports.rldsIngest = exports.rldsSignedUrl = void 0;
 ...
 * }
 */
exports.rldsSignedUrl = functions.https.onRequest(async (req, res) => {
    // CORS
 ...
 * }
 */
exports.rldsIngest = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
    if (req.method === "OPTIONS") {
 ...
 */
exports.rldsList = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    if (req.method !== "GET") {
        res.status(405).json({ error: "Method not allowed" });
        return;
    ...
    try {
        ...
        let query = db.collection("rlds_episodes");
        if (robot_id) {
        ...
        const episodes = snapshot.docs.map(doc => {
            ...
            return (Object.assign(Object.assign({ id: doc.id }, doc.data()), { created_at: (_c = (_b = (_a = doc.data().created_at) === null || _a === void 0 ? void 0 : _a.toDate) === null || _b === void 0 ? void 0 : _b.call(_a)) === null || _c === void 0 ? void 0 : _c.toISOString(), validated_at: (_f = (_e = (_d = doc.data().validated_at) === null || _d === void 0 ? void 0 : _d.toDate) === null || _e === void 0 ? void 0 : _e.call(_d)) === null || _f === void 0 ? void 0 : _f.toISOString() }));
        });
        res.json({
            success: true,
 ...
 * }
 */
exports.remoteVerify = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
    if (req.method === "OPTIONS") {
 ...
 */
exports.remoteSession = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    if (req.method !== "GET") {
        res.status(405).json({ error: "Method not allowed" });
        return;
    ...
    try {
        ...
        const sessionDoc = await db.collection("remote_sessions").doc(session_id).get();
 ...
 * }
 */
exports.trainingCreate = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
    if (req.method === "OPTIONS") {
    ...
    try {
        ...
        const jobId = `job_${Date.now()}_${(0, uuid_1.v4)().substring(0, 8)}`;
 ...
 */
exports.trainingUpdate = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
    if (req.method === "OPTIONS") {
    ...
    try {
        ...
        if (status === "running") {
            ...
            if (model_ref)
                updateData["output.model_ref"] = model_ref;
            if (metrics)
                updateData["output.metrics"] = metrics;
        }
...
exports.rlds = functions.https.onRequest(async (req, res) => {
    const path = req.path.replace("/api/rlds", "");
    if (path === "/signed-url" || path === "/signed-url/") {
        return (0, exports.rldsSignedUrl)(req, res);
    }
    else if (path === "/ingest" || path === "/ingest/") {
        return (0, exports.rldsIngest)(req, res);
    }
    else if (path === "/list" || path === "/list/") {
        return (0, exports.rldsList)(req, res);
    }
    else {
        res.status(404).json({ error: "Endpoint not found" });
    }
});
exports.remote = functions.https.onRequest(async (req, res) => {
    const path = req.path.replace("/api/remote", "");
    if (path === "/verify" || path === "/verify/") {
        return (0, exports.remoteVerify)(req, res);
    }
    else if (path === "/session" || path === "/session/") {
        return (0, exports.remoteSession)(req, res);
    }
    else {
        res.status(404).json({ error: "Endpoint not found" });
    }
});
exports.training = functions.https.onRequest(async (req, res) => {
    const path = req.path.replace("/api/training", "");
    if (path === "/create" || path === "/create/") {
        return (0, exports.trainingCreate)(req, res);
    }
    else if (path === "/update" || path === "/update/") {
        return (0, exports.trainingUpdate)(req, res);
    }
    else {
        res.status(404).json({ error: "Endpoint not found" });
    }
