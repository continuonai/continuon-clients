...
class ContextRetriever:
    ...
    def build_subgraph(
        ...
        """
        Expand a subgraph (nodes + edges) starting from seeds.
        """
        ...
        while frontier and current_depth < depth:
            ...
            for current_id in frontier:
                outbound_edges = self.store.get_outbound_edges(current_id)
    ...
    def recent_subgraph(
        ...
        """
        Build a subgraph seeded by the most recent nodes (optionally filtered by type or tags).
        """
        seeds = [node.id for node in self.store.list_nodes(types=types, limit=limit, tags=tags)]
        if not seeds:
            return {"nodes": [], "edges": []}
        return self.build_subgraph(seeds, depth=depth, min_confidence=min_confidence)
