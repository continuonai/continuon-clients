...
class RobotService:
    ...
    async def RunTraining(self):
        """
        Trigger a background training run using richer torch LoRA hooks.
        Writes status via local_lora_trainer status side-effect.
        """
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, self._run_training_sync)
    ...
    def _run_training_sync(self):
        ...
        class TinyPolicy(nn.Module):
            def __init__(self, obs_dim=OBS_DIM, hidden=HIDDEN, action_dim=2):
                super().__init__()
